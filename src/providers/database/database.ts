...
export class DatabaseProvider {
  ...
  getStorePointsSum(id){
    return this.database.executeSql(`SELECT sp.id, si.capturedAt, sum(sp.points) as total_points
    ...
    getAllStorePointsByStoreImageId(storeImageId) {
      return this.database.executeSql(`SELECT STORE_POINTs.id, STORE_POINTs.id, STORE_POINTs.points, STORE_POINTs.fileName, STORE_IMAGEs.lat as lat
        FROM STORE_POINTs
        INNER JOIN STORE_IMAGEs ON STORE_IMAGEs.id = STORE_POINTs.storeImageId
        WHERE STORE_POINTs.storeImageId = ?`, [storeImageId]).then((data) => {
          let results = [];
          if (data.rows.length > 0) {
            for (var i = 0; i < data.rows.length; i++) {
              results.push({
                ...
                storeImageId: data.rows.item(i).lat
              });
            }
          }
          return results;
